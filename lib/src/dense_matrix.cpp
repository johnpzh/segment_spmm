...
void MT::DenseMatrix::alloc_values(int64_t size) {
  free(values_);
...
void MT::DenseMatrix::alloc_matrix(int64_t num_rows, int64_t num_cols) {
  num_rows_ = num_rows;
  num_cols_ = num_cols;
  alloc_values(num_rows * num_cols);
}

MT::DenseMatrix::DenseMatrix(int64_t num_rows, int64_t num_cols) {
